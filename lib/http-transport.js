...
function _auth( request, options, forceAuthentication ) {
	...
	if (options.authHandler && options.authHandler.auth) {
		// To handle superagent's redirects, we need to override end to ensure
		// we re-sign any subsequent requests
		...
		request.end = function() {
			...
			// Use request object so we don't expose the internal agent to calling script
			var requestObj = {
				getMethod: ()=> this.method,
				getUrl: ()=> this.url,
				setHeader: (key,value)=>this.set(key,value)
			};
			options.authHandler.auth.call(options.authHandler, requestObj, options).then(function() {
					self.__end.apply(self,args);
			});
		};
		return request;
	}
